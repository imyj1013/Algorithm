...
void prim(int n) {
	...
	ggraph.vertex[0].distance = 0;    // 정점 1에서 부터 시작
	H[1].distance = 0;
	int heapsize = n;   // 힙 크기 변수

	for (int i = 0; i < n; i++) {
		int u = H[1].vertnum;     // removemin
		printf(" %d", u);     // 최소신장트리 생성하며 추가된 정점
		sum += H[1].distance;    // 총무게 갱신
		H[1] = H[heapsize];     // removemin
		heapsize--;
		downHeap(heapsize, 1);

		incidence* p = ggraph.vertex[u - 1].incidence->next;   // 인접 정점들 확인하기 위한 포인터
		while (p != NULL) {
			int z = p->adjvert;

			for (int j = 1; j <= heapsize; j++) {    // 인접정점 무게 갱신
				if (H[j].vertnum == z && p->edge->weight < H[j].distance) {
					H[j].distance = p->edge->weight;   // 더 적은 무게로 갱신
					upHeap(j);    // 최소힙으로 정렬
					break;
				}
			}
